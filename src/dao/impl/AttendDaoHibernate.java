...
public class AttendDaoHibernate extends YeekuHibernateDaoSupport implements AttendDao
{
	...
	{
		return (List<Attend>) getHibernateTemplate().find("from Attend as a where a.employee=?", emp);
	}
	...
	{
		return (List<Attend>) getHibernateTemplate().find("from Attend as a where a.employee=? and " + "a.dutyDay=?", new Object[]
		{ emp, dutyDay });
	}
